...
def create_constraints(dataSet):
    for constDict in dataSet:
        ...
        try:
            ...
            warning = "Driven object {0} was not defined or found in the scene".format(driven)
            cmds.confirmDialog(
        ...
        try:
            ...
            warning = "Driver object {0} was not defined or found in the scene".format(driver)
            cmds.confirmDialog(
                title = "Creating Constraints Failed",
                message = warning,
                button = ['OK'],
        ...
        if menuType == "enum":
            cmds.addAttr(uiHost, longName = topMenuName, attributeType = "enum", enumName = menuNames,
                       keyable = True)
            for index, driver in enumerate(drivers):
                ...
                cmds.connectAttr((str(condNode) + "." + "outColor.outColorR"), weightattr)

        if menuType == "float":
            for index, driver in enumerate(drivers):
                cmds.addAttr(uiHost, longName = menuNames[index], attributeType = "float", keyable = True,
                           hasMaxValue = True,
                           hasMinValue = True, maxValue = 1.0, minValue = 0.0)
...
def patch_data(data):
    ...
    supportedKeys = {
        u"drivers": "",
        u"driven": "",
        u"topMenuName": "default",
        u"menuNames": "default",
        u"menuType": "enum",
        u"constraintType": "parentConstraint",
        u"keepOffset": "True",
        u"UIhost": "default"
    }
        ...
        if not sorted(dictionary.keys()) == sorted(supportedKeys.keys()):
            for key in supportedKeys.keys():
                if not key in dictionary.keys():
                    print("{0} key was patched to imported smd data".format(key))
...
class SpaceManager():
    ...
    def __init__(self):
        ...
        self.altGrey = 52.5
        self.constraintTypes = ["parentConstraint", "pointConstraint", "orientConstraint", "scaleConstraint"]
    ...
    def update_connections_table(self):
        ...
        for i in range(self.ui.spaceTable.rowCount()):
            ...
            if self.ui.spaceTable.item(i, 3):
                keepOffset = "True"
                if self.ui.spaceTable.item(i, 3).checkState() == QtCore.Qt.Unchecked:
                    keepOffset = "False"
    ...
    def add_item_to_connections_table(self, drivers = "", driven = "", topMenuName = "default",menuNames = "default",
                                      menuType = "enum", constraintType = "parentConstraint",keepOffset = "True",
                                      UIhost = "default"):
        # new row
        ...
        item.setText(driven)
        item.setBackground(QtGui.QColor(self.altGrey, self.altGrey, self.altGrey))
        ...
        self.ui.spaceTable.setCellWidget(self.ui.spaceTable.rowCount() - 1, 2, conComboBox)
        ...
        item4.setBackground(QtGui.QColor(self.altGrey, self.altGrey, self.altGrey))
        ...
        self.ui.spaceTable.setCellWidget(self.ui.spaceTable.rowCount() - 1, 6, typeComboBox)
    ...
    def add_driven_click(self):
        ...
        for eachItem in selection:
            # adds entry
            self.add_item_to_connections_table(drivers = "", driven = eachItem, constraintType = "parentConstraint",
                                               menuType = "enum", keepOffset = "True")
            cmds.select(selection, r = True)

    def add_drivers_click(self):
        selection = cmds.ls(sl = True)
        self.add_item_to_connections_table(drivers = selection, driven = "", constraintType = "parentConstraint",
                                           menuType = "enum", keepOffset = "True")
        cmds.select(selection, r = True)
        ...
        if len(selection) >= 2:
            ...
            for i in range(self.ui.spaceTable.rowCount()):
                if self.ui.spaceTable.item(i, 1).text() == driven:
                    cmds.warning("Item '{}' is already on the list as a 'driven' item.".format(driven))
                    return
            # if we didn't exit because the driven is already on the list then we add the entry
            self.add_item_to_connections_table(drivers = drivers, driven = driven, constraintType = "parentConstraint",
                                               menuType = "enum", keepOffset = "True")
        else:
            cmds.warning("Need to select at least two items to add as 'drivers' and 'driven'.")
        cmds.select(selection, r = True)
    ...
    def add_ui_host_click(self):
        ...
        if selected_rows:
            ...
            self.add_item_to_connections_table(drivers = "", driven = "", constraintType = "parentConstraint",
                                               keepOffset = "True", UIhost = selection[0])
        cmds.select(selection, r = True)
    ...
    def save_table_as_json(self):
        ...
        path = cmds.fileDialog2(caption="select folder", dialogStyle=1, fileMode=0,fileFilter=smdFilter,returnFilter=True)
        fullpath = str(path[0])
    ...
    def show_data_on_table(self, data):
        ...
        for index,dictionary in enumerate(data):
            ...
            item2.setBackground(QtGui.QColor(self.altGrey, self.altGrey, self.altGrey))
            ...
            item5.setBackground(QtGui.QColor(self.altGrey, self.altGrey, self.altGrey))

            # menu names
            item6.setText(str(dictionary["menuNames"]))
            item6.setBackground(QtGui.QColor(self.altGrey, self.altGrey, self.altGrey))
            ...
            item8.setBackground(QtGui.QColor(self.altGrey, self.altGrey, self.altGrey))
    ...
    def import_data_dialog(self, debug="True"):
        ...
        smdFilter = "Space Manager Dictionary (*.smd)"
        path = cmds.fileDialog2(caption="select file", dialogStyle=1,fileMode=1,fileFilter=smdFilter,returnFilter=True)
        path = path[0]
        with open(path, "r") as read_file:
            # Convert JSON file to Python Types
            obj = json.load(read_file)
    ...
    def set_ui_connections(self):
        ...
        self.ui.clearBtn.clicked.connect(partial(self.clear_table, self.ui.spaceTable))
        self.ui.removeRowBtn.clicked.connect(partial(self.remove_row, self.ui.spaceTable))
