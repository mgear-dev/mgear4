...
class AnimClipsListWidget(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self._add_anim_clip_button = QtWidgets.QPushButton(
            "Add Clip", parent=self
        )
        self._delete_all_clips_button = QtWidgets.QPushButton(
            "Delete All Clips", parent=self
        )
    ...
    def _add_animation_clip(self, clip_name):
        ...
        anim_clip_widget = AnimClipWidget(
            clip_name, parent=self._game_tools_fbx
        )
    ...
    def _on_add_animation_clip_button_clicked(self):
        ...
        if not self._root_joint:
            cmds.warning(
                "Was not possible to add animation clip because not root joint is defined!"
            )
            return
...
class AnimClipWidget(QtWidgets.QFrame):
    def __init__(self, clip_name=None, parent=None):
        ...
        self._title_line_edit.setSizePolicy(
        ...
        self._anim_layer_label = QtWidgets.QLabel(
            "Animation Layer: ", parent=self
        )
        ...
        self._start_frame_line_edit.textChanged.connect(
            self._on_update_anim_clip
        )
        self._end_frame_line_edit.textChanged.connect(
            self._on_update_anim_clip
        )
    ...
    def _on_close_button_clicked(self):
        ...
        root_joint = (
            self._game_tools_fbx.get_root_joint()
            if self._game_tools_fbx
            else None
        )
        if not root_joint:
            return
    ...
    def _on_export_button_clicked(self):
        ...
        export_path = (
            self._game_tools_fbx.get_export_path()
            if self._game_tools_fbx
            else ""
        )
        file_name = (
            self._game_tools_fbx.get_file_name()
            if self._game_tools_fbx
            else ""
        )
        remove_namespace = (
            self._game_tools_fbx.get_remove_namespace()
            if self._game_tools_fbx
            else False
        )
        scene_clean = (
            self._game_tools_fbx.get_scene_clean()
            if self._game_tools_fbx
            else False
        )
        anim_layer = self._anim_layer_combo.currentText()
        ...
        export_kwargs = {
            ...
            "file_name": "{}_{}".format(
                file_name, self._title_line_edit.text().replace(" ", "_")
            ),
