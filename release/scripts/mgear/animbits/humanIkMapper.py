...
class HumanIKMapper:
    ...
    def set_sub_ik(cls, bone_target, sub_ik):
        ...
        pm.connectAttr(
            "{}.sub_ik".format(sub_ik), "{}.sub_ik".format(bone_target)
        )
    ...
    def clear_sub_ik(cls, bone_target, sub_ik):
        if pm.attributeQuery(
            "sub_ik", node=bone_target, exists=True
        ) and pm.attributeQuery("sub_ik", node=sub_ik, exists=True):
            pm.disconnectAttr(
                "{}.sub_ik".format(sub_ik), "{}.sub_ik".format(bone_target)
            )
