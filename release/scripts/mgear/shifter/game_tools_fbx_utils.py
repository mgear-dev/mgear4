...
TRANSFORM_ATTRIBUTES = ['tx', 'ty', 'tz', 'rx', 'ry', 'rz', 'sx', 'sy', 'sz', 'visibility']
...
class FbxExportNode(object):
	...
	def get_animation_clips(self, root_joint_name):
		return self._parse_export_data().get('animClips', dict()).get(root_joint_name, list())
	...
	def add_animation_clip(self, root_joint_name, anim_clip_data=None):
		...
		clip_data.update(anim_clip_data if anim_clip_data is not None else dict())
	...
	def _add_attributes(self):
		...
		if not cmds.attributeQuery(self.EXPORT_DATA_ATTR, n=self._node, ex=True):
			...
			try:
				...
				cmds.warning('Error while converting FbxExportNode data into a dictionary')
				return False
...
def export_skeletal_mesh(jnt_roots, geo_roots, **export_data):
	...
	if geo_roots:
		for geo_root in geo_roots:
			meshes = cmds.listRelatives(geo_root, children=True, type='transform') or list()
...
def export_skeletal_mesh_partitions(jnt_roots, **export_data):
	...
	for partition_name, meshes in partitions.items():
		...
		partitions_data[partition_name]['hierarchy'] = [jnt.name() for jnt in short_hierarchy]
	...
	try:
		for partition_name, partition_data in partitions_data.items():
			...
			fbx_file.export_skeletal_mesh(
				file_name=partition_name, mesh_names=partition_meshes, hierarchy_joints=partition_data['hierarchy'],
				deformations=deformations, skins=skinning, blendshapes=blendshapes)
			fbx_file.close()
	except Exception:
...
def export_animation_clip(root_joint, **export_data):
	...
	if start_frame > end_frame:
		cmds.error('Start frame {} must be lower than the end frame {}'.format(start_frame, end_frame))
		return False
	...
	if not file_path or not file_name:
		cmds.warning('No valid file path or file name given for the FBX to export!')
		return False
	if title:
	...
	try:
		...
		if anim_layer and cmds.objExists(anim_layer) and cmds.nodeType(anim_layer) == 'animLayer':
			to_activate = None
			to_deactivate = []
			anim_layers = all_anim_layers_ordered(include_base_animation=False)
			original_anim_layer_weights = {anim_layer: cmds.animLayer(anim_layer, query=True, weight=True) for anim_layer in anim_layers}
...
def create_mgear_playblast(file_name='', folder=None, start_frame=None, end_frame=None, scale=75):
	...
	if not folder or not os.path.isdir(folder):
		...
		if not os.path.isdir(folder):
			os.makedirs(folder)
	if not os.path.isdir(folder):
		cmds.warning('Was not possible to create mgear playblasts folder: "{}"'.format(folder))
		return False
	full_path = os.path.join(folder, file_name)
	count = 1
	while os.path.isfile(full_path):
		_file_name = '{}_{}{}'.format(os.path.splitext(file_name)[0], count, os.path.splitext(file_name)[1])
...
def open_mgear_playblast_folder():
	folder = get_mgear_playblasts_folder()
	if not folder or not os.path.isdir(folder):
		cmds.warning('Was not possible to open mgear playblasts folder: "{}"'.format(folder))
		return False
