...
class HumanIKMapper():
        ...
        for i in range(hik_count):
            ...
            if bone_target:
                ...
                if pm.attributeQuery("sub_ik", node=bone_target, exists=True):
                    connections = cmds.listConnections("{}.sub_ik".format(bone_target), source=0, destination=1)
                    if connections:
    ...
    def import_char_configuration(cls):
        ...
        ctls = [pm.PyNode(cls.char_config[bone]['target']) for bone in cls.char_config]
    ...
    def set_sub_ik(cls, bone_target, sub_ik_ctls):
        ...
        for sub_ik_ctl in sub_ik_ctls:
            ...
            if not pm.attributeQuery("sub_ik", node=sub_ik_ctl, exists=True):
                pm.addAttr(sub_ik_ctl, longName="sub_ik", attributeType='message')
    ...
    def clear_sub_ik(cls, bone_target):
        sub_ik_ctls = pm.listConnections("{}.sub_ik".format(bone_target), source=0, destination=1)

        if sub_ik_ctls:
            for sub_ik_ctl in sub_ik_ctls:
                if pm.attributeQuery("sub_ik", node=sub_ik_ctl, exists=True):
                    pm.disconnectAttr("{}.sub_ik".format(bone_target), "{}.sub_ik".format(sub_ik_ctl))
    ...
    def bake(cls):
        ...
        '''.format(current_ik_char, attrs_string)
    ...
    def get_sub_ik_bake_attrs(cls):
        ...
        for bone in cls.char_config:
            for sub_ik in cls.char_config[bone]["sub_ik"]:
                ...
                t_locked = [attr.get(settable=1) for attr in sub_ik.translate.children()]
                r_locked = [attr.get(settable=1) for attr in sub_ik.rotate.children()]
...
class HumanIKMapperUI(MayaQWidgetDockableMixin, QtWidgets.QDialog):
    ...
    def create_connections(self):
        ...
        self.right_arm_btn.clicked.connect(
            self.display_list_mb_cb(self._change_bones_2_right(HumanIKMapper.LEFT_ARM_NAMES)))
        self.right_upper_arm_rolls_btn.clicked.connect(
            self.display_list_mb_cb(self._change_bones_2_right(HumanIKMapper.LEFT_UPPER_ARM_ROLLS)))
        self.right_lower_arm_rolls_btn.clicked.connect(
            self.display_list_mb_cb(self._change_bones_2_right(HumanIKMapper.LEFT_LOWER_ARM_ROLLS)))
        self.right_hand_btn.clicked.connect(
            self.display_list_mb_cb(self._change_bones_2_right(HumanIKMapper.LEFT_HAND)))
        ...
        self.right_leg_btn.clicked.connect(
            self.display_list_mb_cb(self._change_bones_2_right(HumanIKMapper.LEFT_LEG_NAMES)))
        self.right_upper_leg_rolls_btn.clicked.connect(
            self.display_list_mb_cb(self._change_bones_2_right(HumanIKMapper.LEFT_UPPER_LEG_ROLLS)))
        self.right_lower_leg_rolls_btn.clicked.connect(
            self.display_list_mb_cb(self._change_bones_2_right(HumanIKMapper.LEFT_LOWER_LEG_ROLLS)))
        ...
        self.sub_ik_active_cb.stateChanged.connect(HumanIKMapper.sub_iks_binding)
    ...
    def _mapping_menu(self, QPos):
        ...
        clear_sub_ik.triggered.connect(partial(self.clear_sub_ik, corresponding_bone))
